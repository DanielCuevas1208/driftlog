...
pub type Replica {
  Replica(name: String, text: List(Edit), register: List(Edit))
}
...
fn scenario_decoder() -> decode.Decoder(Scenario) {
  ...
  decode.success(Scenario(
    ...
    base:,
    replicas:,
    expected_text: pair.first(expected),
    expected_register: pair.second(expected),
  ))
}

fn expected_decoder() -> decode.Decoder(#(String, String)) {
  use text <- decode.field("text", decode.string)
  use register <- decode.field("register", decode.string)
  decode.success(#(text, register))
}
...
fn replica_decoder() -> decode.Decoder(Replica) {
  ...
  decode.success(Replica(name:, text:, register:))
}
